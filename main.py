...
from app import db
# 🔥 FORCE CREATE DATABASE TABLES IMMEDIATELY
...
# 🔍 DEBUG: verify Pinecone + model dimensions
print("🔍 INDEX NAME:", PINECONE_INDEX_NAME)
print("🔍 INDEX DIM:", index.describe_index_stats()["dimension"])
print("🔍 MODEL DIM:", len(model.encode("hello")))

# 🔐 Safety check: ensure Pinecone index dimension matches embedding model
index_stats = index.describe_index_stats()
expected_dim = len(model.encode("test"))

if index_stats["dimension"] != expected_dim:
    raise RuntimeError(
        f"Pinecone index dimension {index_stats['dimension']} "
...
async def upload_documents(
    ...
    db: Session = Depends(db.get_db_connection)
):
